...
| 自己生成自己检查就行 | 主 agent 自评必然偏高 | 验证全过但用户一测就穿帮 | 行为 | 结构推断 | Phase 4 spawn 子 agent 独立测试 | stage-06 / A8 |
...
| 字幕下载清洗手工搞 | 下载/清洗易错且重复 | 字幕带时间戳和重复行混进语料污染分析 | 领域-工程 | 结构推断 | scripts/ 四个脚本接管机械活 | stage-03 / A11 |
...
- A8（自评偏差）和 A11（字幕坑）证据等级为结构推断——SKILL.md 没有明说踩过坑，是从设计反推的，标注如实。
