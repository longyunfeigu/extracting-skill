(function () {
  ...
  function buildChapters() {
    ...
    return [
      ...
      { label: "Walkthrough", sub: `AI 运行轨迹 · ${counts.stages} 个 stage`, href: `${root}pages/walkthrough.html`, slug: "walkthrough" },
  ...
  function getSubNav(page) {
    switch (page) {
      case "overview":
        return [
          { anchor: "scene", label: "先看 AI 默认怎么坏" },
    ...
    layout("Glossary", `
      <article class="page">
        <header class="wt-hero">
          ...
          <h1>概念词典</h1>
          <p class="lede">${terms.length} 个核心术语，每条 5 个字段——人话解释 / 出现场景 / 解决什么问题 / 我作为 AI 怎么用 / 容易误解。Walkthrough 里的就地短解保证当下读得动；这一页给想系统过一遍术语的人用。</p>
          <span class="hero-rule"></span>
        </header>
          ...
          <div class="card-grid">
            ${terms.map((t) => `
              <article class="card glossary-card" id="${slugify(t.term)}">
                ...
                <div class="card-row"><span class="label">我作为 AI 怎么用它</span><p>${escapeHtml(t.howToUse || "")}</p></div>
